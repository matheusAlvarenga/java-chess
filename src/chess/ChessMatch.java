...
public class ChessMatch {
  ...
  public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
    ...
    check = (isInCheck(getOpponentColor(currentPlayer))) ? true : false;

    checkmate = (isInCheckmate(getOpponentColor(currentPlayer))) ? true : false;
  ...
  private Piece makeMove(Position source, Position target) {
    ...
    if(p instanceof Pawn) {
      if(source.getColumn() != target.getColumn() && capturedPiece == null) {
        int rowsCalc = p.getColor() == Color.WHITE ? 1 : -1;
        
        Position pawnPosition = new Position(target.getRow() + rowsCalc, target.getColumn());
  ...
  private void undoMove(Position source, Position target, Piece capturedPiece) {
    ...
    if(p instanceof Pawn) {
      if(source.getColumn() != target.getColumn() && capturedPiece == enPassantVulnerable) {
        ...
        pawnPosition = new Position(source.getRow() + rowsCalc, target.getColumn());
      ...
      for(int i = 0; i < board.getRows(); i++) {
        for(int j = 0; j < board.getColumns(); j++) {
          if(mat[i][j]) {
            ...
            boolean testCheck = isInCheck(color);
